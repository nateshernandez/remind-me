...
import { callbackOutcome } from "@/lib/access/routing"

// RULE-access-callback-outcome: the markdown table is the artifact a reviewer signs; this
...
const table = parseDecisionTable(
  readFileSync(
    join(import.meta.dirname, "RULE-access-callback-outcome.md"),
    "utf8"
  )
)

describe("RULE-access-callback-outcome", () => {
  it("agrees with the table in every region it distinguishes", () => {
    ...
    for (const input of regions) {
      expect({ input, got: callbackOutcome(input as never) }).toEqual({
        input,
